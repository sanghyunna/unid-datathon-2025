...
class CrossAttentionLayer(nn.Module):
    def __init__(self, embed_dim: int, attn_dim: int, heads: int):
        super().__init__()
        if attn_dim <= 0:
            raise ValueError("cross_attn_dim must be positive")
        if heads <= 0:
            raise ValueError("cross_attn_heads must be positive")
        if attn_dim % heads != 0:
            raise ValueError("cross_attn_dim must be divisible by cross_attn_heads")
        ...
        self.v_proj = nn.Conv2d(embed_dim, attn_dim, 1)
    ...
    def forward(self, q_vec: torch.Tensor, fmap: torch.Tensor) -> torch.Tensor:
        ...
        K = self.k_proj(fmap).view(B, self.heads, self.head_dim, -1)
        V = self.v_proj(fmap).view(B, self.heads, self.head_dim, -1)
...
class CrossAttnVLM(nn.Module):
    def __init__(self, vocab_size: int, dim: int = CFG.DIM, pretrained_backbone: bool = True,
        ...
        self.num_cross_attn = layers  # backward-compat field name used elsewhere
